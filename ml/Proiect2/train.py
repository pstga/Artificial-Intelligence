...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=80, fold=1,
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        # Validare cu TTA folosind modelul EMA
        ema.ema.eval()
        val_correct = 0
        total_val = 0
        with torch.no_grad():
        ...
        print(
            f"Epoch [{epoch + 1}/{num_epochs}] Train Loss: {avg_train_loss:.4f} | Train Acc: {train_accuracy:.2f}% | Val Acc (TTA EMA): {val_accuracy:.2f}%")

        if val_accuracy > best_val_acc:
            best_val_acc = val_accuracy
            save_path = os.path.join(save_dir, f'fold_{fold}_best_model.pth')
            torch.save(ema.ema.state_dict(), save_path)
            print(f"  ---> [Saved new best EMA model for Fold {fold}!]")
