...
class AddFreqCoords(nn.Module):
    ...
    def forward(self, x):
        ...
        # Only add frequency (y-axis) coordinates
...
class SEBlock(nn.Module):
    def __init__(self, channels, reduction=16):
        ...
        # Ensure reduction doesn't drop channels below 1
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply SE Attention right before adding the shortcut
        out = self.se(out)
...
class CustomResNet(nn.Module):
    def __init__(self, num_classes):
        ...
        self.layer3 = ResidualBlock(in_channels=128, out_channels=128, stride=1)

        self.spatial_pool = nn.AdaptiveAvgPool2d((4, 4))
        self.dropout = nn.Dropout(p=0.3)

        self.channel_reducer = nn.Conv2d(in_channels=128, out_channels=16, kernel_size=1)
